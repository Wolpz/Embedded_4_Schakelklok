...
/*******************************************************************************
* Function Name: mSec_clock_2_Start
********************************************************************************
*
* Summary:
...
void mSec_clock_2_Start(void) 
{
    ...
    mSec_clock_2_CLKEN |= mSec_clock_2_CLKEN_MASK;
	mSec_clock_2_CLKSTBY |= mSec_clock_2_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: mSec_clock_2_Stop
********************************************************************************
*
* Summary:
...
void mSec_clock_2_Stop(void) 
{
    ...
    mSec_clock_2_CLKEN &= (uint8)(~mSec_clock_2_CLKEN_MASK);
	mSec_clock_2_CLKSTBY &= (uint8)(~mSec_clock_2_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: mSec_clock_2_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((mSec_clock_2_CLKEN & mSec_clock_2_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = mSec_clock_2_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = mSec_clock_2_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* mSec_clock_2__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(mSec_clock_2_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(mSec_clock_2_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: mSec_clock_2_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: mSec_clock_2_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = mSec_clock_2_GetSourceRegister();
    uint16 oldDivider = mSec_clock_2_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = mSec_clock_2_CLKEN & mSec_clock_2_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(mSec_clock_2_DIV_PTR, clkDivider);
                mSec_clock_2_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                mSec_clock_2_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(mSec_clock_2_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((mSec_clock_2_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: mSec_clock_2_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(mSec_clock_2_DIV_PTR);
}


/*******************************************************************************
* Function Name: mSec_clock_2_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    mSec_clock_2_MOD_SRC |= modeBitMask & (uint8)mSec_clock_2_MODE_MASK;
}


/*******************************************************************************
* Function Name: mSec_clock_2_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    mSec_clock_2_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(mSec_clock_2_MODE_MASK));
}


/*******************************************************************************
* Function Name: mSec_clock_2_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return mSec_clock_2_MOD_SRC & (uint8)(mSec_clock_2_MODE_MASK);
}


/*******************************************************************************
* Function Name: mSec_clock_2_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = mSec_clock_2_GetDividerRegister();
    uint8 oldSrc = mSec_clock_2_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        mSec_clock_2_MOD_SRC |= CYCLK_SSS;
        mSec_clock_2_MOD_SRC =
            (mSec_clock_2_MOD_SRC & (uint8)(~mSec_clock_2_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: mSec_clock_2_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return mSec_clock_2_MOD_SRC & mSec_clock_2_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: mSec_clock_2_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    mSec_clock_2_PHASE = clkPhase & mSec_clock_2_PHASE_MASK;
}


/*******************************************************************************
* Function Name: mSec_clock_2_GetPhase
********************************************************************************
*
* Summary:
...
{
    return mSec_clock_2_PHASE & mSec_clock_2_PHASE_MASK;
}
